...
export const renderSoundButton = (root: HTMLElement) => {
    root.innerHTML += `
    <button id="audio-toggler" class="z-50 rounded-full flex place-items-center absolute bg-neutral-400 shadow-2xl bottom-0 right-0 m-2 text-neutral-800 p-2 border h-10 w-10"></button>
    <audio id="coin-audio">
...
const setProperlySoundIcon = (btnAudioElement: HTMLButtonElement, isAllowed: Boolean) => {
    if(isAllowed) {
    btnAudioElement.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 24 24">
    ...
    btnAudioElement.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 25 24">
...
const unlockAudio = (player: HTMLAudioElement) => {
...
export const playSound = (player: HTMLAudioElement) => {
    if(player && playAudioIsAllowed) {
        player.play();
    }
}

export const getCoinAudioPlayer = () => {
    return document.querySelector("#coin-audio") as HTMLAudioElement;
}

let playAudioIsAllowed = false;
const initSoundButton = (element: HTMLElement) => {
    const btnAudioToggler = element.querySelector('#audio-toggler') as HTMLButtonElement;
    setProperlySoundIcon(btnAudioToggler, playAudioIsAllowed);
    element.addEventListener('click', () => {
        ...
        if (playAudioIsAllowed && !audioUnlocked) {
            ...
            unlockAudio(getCoinAudioPlayer())
        }
        setProperlySoundIcon(btnAudioToggler, playAudioIsAllowed);
    })
}
